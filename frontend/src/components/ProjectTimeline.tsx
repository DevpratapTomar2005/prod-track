...
> = {
  Done: {
    dot: "#22c55e",
    border: "#16a34a",
    glow: "0 0 0 5px #bbf7d050,0 0 16px #22c55e66",
    lineColor: "#22c55e",
    label: "Done",
    badgeBg: "#dcfce7",
    badgeText: "#15803d",
  ...
  "In Progress": {
    dot: "#00d3f2",
    border: "#0891b2",
    glow: "0 0 0 5px #a5f3fc50,0 0 16px #00d3f288",
    lineColor: "#00d3f2",
    label: "In Progress",
    badgeBg: "#cffafe",
    badgeText: "#0e7490",
...
const TooltipPortal = ({
  ...
  const estimatedH =
    data.kind === "gap" ? 110 : data.entry.overrunHours > 0 ? 175 : 155;
  ...
  if (data.kind === "gap") {
    ...
    card = (
      <div
        className="bg-gray-950 border border-red-800/50 rounded-xl px-3.5 py-3 shadow-2xl w-52"
        ...
        <div className="space-y-1 text-[10px] text-gray-400">
          <div className="flex justify-between">
            ...
            <span className="text-gray-200">
              {fmtDate(gapStart)} {fmtTime(gapStart)}
            </span>
          </div>
          <div className="flex justify-between">
            <span>To</span>
            <span className="text-gray-200">
              {fmtDate(gapEnd)} {fmtTime(gapEnd)}
            </span>
          </div>
          <div className="flex justify-between border-t border-gray-800 pt-1 mt-1">
            <span>Idle</span>
            <span className="text-red-400 font-semibold">{gapH}h</span>
          </div>
        </div>
      </div>
    );
    ...
    card = (
      <div
        className="bg-gray-950 border border-gray-700/60 rounded-xl px-3.5 py-3 shadow-2xl w-56"
        style={{ fontFamily: "Poppins, sans-serif" }}
      >
        <p
        ...
        <div className="space-y-1 text-[10px] text-gray-400">
          <div className="flex justify-between gap-3">
            <span>Start</span>
            <span className="text-gray-200">
              {fmtDate(entry.actualStart)} {fmtTime(entry.actualStart)}
            </span>
          </div>
          <div className="flex justify-between gap-3">
            <span>Planned end</span>
            <span className="text-gray-200">
              {fmtDate(entry.plannedEnd)} {fmtTime(entry.plannedEnd)}
            </span>
          </div>
          {entry.task.status !== "To Do" && (
            <div className="flex justify-between gap-3">
              <span>
                {entry.task.status === "In Progress" ? "Current" : "Actual end"}
              </span>
              <span
                className={
                  entry.overrunHours > 0 ? "text-orange-400" : "text-green-400"
                }
              >
                {fmtDate(entry.actualEnd)} {fmtTime(entry.actualEnd)}
              </span>
            </div>
          )}
          <div className="flex justify-between gap-3">
            ...
            <span className="text-gray-200">
              {parseHours(
                entry.task.estDuration,
                entry.task.estDurationUnit,
              ).toFixed(1)}
              h planned
            </span>
          </div>
          {entry.overrunHours > 0 && (
            <div className="flex justify-between gap-3 border-t border-gray-800 pt-1 mt-1">
              ...
              <span className="text-orange-400 font-semibold">
                +{entry.overrunHours.toFixed(1)}h
              </span>
            </div>
          )}
        </div>
...
const Connector = ({
  ...
  >
    ...
    {showGapTooltip && gap && entry && (
      <TooltipPortal
        data={{ kind: "gap", entry, gapStart: gap.start, gapEnd: gap.end }}
        x={mouseX}
        y={mouseY}
      />
    )}
...
const MilestoneNode = ({
  ...
  <div
    ...
    style={{ width: 44, zIndex: 10 }}
  >
    {/* Label */}
    {labelAbove ? (
      <div
        className="absolute bottom-[calc(100%+10px)] left-1/2 -translate-x-1/2 flex flex-col items-center text-center"
        style={{ width: 110 }}
      >
        <span
        ...
        <p
          className="text-[9px] text-gray-700 font-semibold leading-tight mb-0.5"
          style={{ fontFamily: "Poppins, sans-serif" }}
        >
          {title}
        </p>
      ...
      <div
        className="absolute top-[calc(100%+10px)] left-1/2 -translate-x-1/2 flex flex-col items-center text-center"
        style={{ width: 110 }}
      >
        ...
        <p
          className="text-[9px] text-gray-700 font-semibold leading-tight mb-1"
          style={{ fontFamily: "Poppins, sans-serif" }}
        >
          {title}
        </p>
        {subtitle && (
          <p
            className="text-[8px] text-gray-500 mb-1"
            style={{ fontFamily: "Poppins, sans-serif" }}
          >
            {subtitle}
          </p>
    ...
    >
      <span
        style={{
          transform: "rotate(-45deg)",
          fontSize: 13,
          color: active ? "white" : borderColor,
          fontWeight: 700,
          lineHeight: 1,
          display: "block",
        }}
...
export const ProjectTimeline = ({ tasks }: { tasks: Task[] }) => {
  ...
  const entries: Entry[] = tasks
    .map((t) => {
      ...
      const plannedEnd = new Date(
        actualStart.getTime() + durationHours * 3600000,
      );
      let actualEnd: Date;
      if (t.status === "Done") {
      ...
      const overrunHours =
        t.status !== "To Do" && actualEnd > plannedEnd
          ? (actualEnd.getTime() - plannedEnd.getTime()) / 3600000
          : 0;
  ...
  const gaps = entries.map((e, i) => {
    ...
    if (nextStart.getTime() > currEnd.getTime() + 60000) {
      return {
        ...
        hours: (nextStart.getTime() - currEnd.getTime()) / 3600000,
      };
    }
    return null;
  });

  const isReached = (e: Entry) =>
    e.task.status === "Done" || e.task.status === "In Progress";
  ...
  const trackMove = (e: React.MouseEvent) =>
    setMousePos({ x: e.clientX, y: e.clientY });

  return (
    <div className="w-[90%] max-[700px]:w-[90%] max-w-[1050px] mx-auto mt-6 mb-10 bg-white border border-gray-200 rounded-lg shadow">
      {/* ── Header ── */}
      <div className="flex items-center justify-between max-[700px]:flex-col max-[700px]:items-start mb-4 p-4 gap-3">
        <div>
          <h2 className="text-[15px] font-bold text-gray-800 font-inter">
            Project Timeline
          </h2>
          <p
            ...
            style={{ fontFamily: "Poppins, sans-serif" }}
          >
            Task execution track · Hover nodes for details
          </p>
        </div>
        <div
          className="flex items-center gap-4 max-[700px]:flex-wrap max-[700px]:gap-x-3 max-[700px]:gap-y-1.5 text-[10px] text-gray-500"
          style={{ fontFamily: "Poppins, sans-serif" }}
        >
          {Object.entries(STATUS_CFG).map(([k, v]) => (
            <span key={k} className="flex items-center gap-1.5">
              <span
                ...
                style={{ background: v.dot, borderColor: v.border }}
              />
              {v.label}
            </span>
          ))}
      ...
      <div className="bg-white py-3 px-2">
        <div
          className="tl-scroll no-scrollbar max-[700px]:px-4"
          style={{
            overflowX: "auto",
            overflowY: "hidden",
            paddingTop: 100,
        ...
        >
          ...
          <div
            ...
            style={{
              paddingTop: 80,
              paddingBottom: 80,
              minWidth: Math.max((entries.length + 2) * 120, 620),
            }}
          >
            {/* ── Row of nodes ── */}
            <div className="flex items-center w-full">
              ...
              {entries.map((entry, i) => {
                const cfg =
                  STATUS_CFG[entry.task.status] ?? STATUS_CFG["To Do"];
                  ...
                  <React.Fragment key={entry.task.id}>
                    ...
                    >
                      ...
                      {labelAbove && (
                        ...
                        >
                          ...
                          <p
                            className="text-[9px] text-gray-700 font-semibold leading-tight mb-0.5"
                            style={{ fontFamily: "Poppins, sans-serif" }}
                            title={entry.task.task}
                          >
                            {entry.task.task.length > 16
                              ? entry.task.task.slice(0, 16) + "…"
                              : entry.task.task}
                          </p>
                          <p
                            className="text-[8px] text-gray-400"
                            style={{ fontFamily: "Poppins, sans-serif" }}
                          >
                            {fmtTime(entry.actualStart)}
                          </p>
                          {entry.overrunHours > 0 && (
                            <p
                              className="text-[8px] text-orange-500 font-semibold mt-0.5"
                              style={{ fontFamily: "Poppins, sans-serif" }}
                            >
                              +{entry.overrunHours.toFixed(1)}h over
                            </p>
                          )}
                          <div
                            className="w-px bg-gray-200 mt-1"
                            style={{ height: 10 }}
                          />
                        </div>
                      )}
                      ...
                      {!labelAbove && (
                        ...
                        >
                          <div
                            className="w-px bg-gray-200 mb-1"
                            style={{ height: 10 }}
                          />
                          <p
                            className="text-[8px] text-gray-400 mb-0.5"
                        ...
                        {isActive && (
                          <span
                            ...
                            style={{
                              ...
                              animation:
                                "tlPing 1.4s cubic-bezier(0,0,0.2,1) infinite",
                            }}
                          />
                        )}
                        <div
            ...
            <div
              className="absolute -bottom-6 flex items-start w-full"
              style={{ left: 0 }}
            >
              ...
              {entries.map((entry, i) => (
                <React.Fragment key={entry.task.id}>
                  ...
                  >
                    ...
                    <span
                      className="text-[8px] text-gray-400 mt-0.5 whitespace-nowrap"
                      style={{ fontFamily: "Poppins, sans-serif" }}
                    >
      ...
      {/* ── Summary strip ── */}
      <div
        className="mt-7 pt-4 border-t border-gray-100 flex items-center flex-wrap gap-x-6 gap-y-1.5 max-[700px]:gap-x-3 text-[10px] text-gray-500 p-4"
        style={{ fontFamily: "Poppins, sans-serif" }}
      >
        <span>
          <span className="font-semibold text-gray-700">
            {entries.filter((e) => e.task.status === "Done").length}
          </span>{" "}
          of{" "}
        ...
        {entries.some((e) => e.overrunHours > 0) && (
          <span className="text-orange-500">
            ⚠ {entries.filter((e) => e.overrunHours > 0).length} task
            {entries.filter((e) => e.overrunHours > 0).length > 1 ? "s" : ""}{" "}
            overran schedule
          </span>
        )}
        {gaps.some(Boolean) && (
          <span className="text-red-500">
            {gaps.filter(Boolean).length} unscheduled gap
            {gaps.filter(Boolean).length > 1 ? "s" : ""} detected
          </span>
        )}
        <span className="ml-auto max-[700px]:ml-0 max-[700px]:basis-full text-gray-400">
          {fmtDate(entries[0].actualStart)} –{" "}
          {fmtDate(entries[entries.length - 1].plannedEnd)}
        </span>
      </div>
